...
export async function syncUserToConvex(user: BetterAuthUser): Promise<ConvexSyncResponse | null> {
  try {
    ...
    console.log(`✅ Successfully synced user to Convex: ${result.action} user ${user.email}`);
    return {
      ...result,
